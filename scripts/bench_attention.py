...
def main() -> None:
    ...
    print(f"device={torch.cuda.get_device_name(0)} | PicoLM attention: H={H}, D={D}, fp16")
    ...
    for T in (64, 128, 256, 512, 1024):
        ...
        ms_flash = triton.testing.do_bench(lambda: flash_attention(q, k, v))

        print(f"{T:>5} {ms_eager:>8.3f}m {ms_flash:>8.3f}m {ms_eager / ms_flash:>7.2f}x")
